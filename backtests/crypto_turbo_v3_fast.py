...
OUT=Path('turbo_v3_fast_output'); OUT.mkdir(exist_ok=True)
...
def build_arrays():
    ...
    for j,s in enumerate(SYMS):
        p=ps[s]
        ii=np.array([pos[t] for t in p.index],dtype=int)
...
def test_window(idx,A,cfg,i0,i1):
    ...
    for i in range(i0+1,i1+1):
        p=i-1
        c0=A['close'][p]; c1=A['close'][i]; score=A['score'][p]; accel=A['accel'][p]; bull=A['bull'][p]; ultra=A['ultra'][p]
        ...
        if len(inds):
            ...
            conc=len(top)==1
            if len(top)>=2: conc=(score[top[0]]-score[top[1]]>=cfg['edge']) and (accel[top[0]]>0)
            if conc:
                desired[top[0]]=cfg['single']
            else:
                desired[top]=cfg['pair_total']/len(top)
            for j in top:
        ...
        dd=eq/peak-1
        if dd<=-0.20: desired*=0.25
        elif dd<=-0.10: desired*=0.60
        # trailing locks
        held=np.where(np.isfinite(entry))[0]
        for j in held:
            if np.isfinite(c0[j]):
                tpeak[j]=max(tpeak[j] if np.isfinite(tpeak[j]) else c0[j],c0[j])
                if tpeak[j]/entry[j]-1>=0.25 and c0[j]/tpeak[j]-1<=-cfg['trail']:
                    desired[j]=0.0
        tot=desired.sum()
...
def main():
    idx,A=build_arrays()
    start0=max(idx.min(),pd.Timestamp('2018-01-01',tz='UTC')); last=idx.max()-pd.Timedelta(days=730)
    starts=pd.date_range(start0,last,freq='180D')
    cfgs=[]
    for single,pair_total in [(0.8,1.0),(1.0,1.2)]:
      for ultra,p1,p2 in [(1.5,1.25,1.75),(2.0,1.5,2.25)]:
       for trail in [0.12,0.18]:
        cfgs.append({'single':single,'pair_total':pair_total,'ultra':ultra,'p1':p1,'p2':p2,'trail':trail,'edge':0.035,'max_total':2.5})
    rows=[]; grids=[]
    for ci,cfg in enumerate(cfgs,1):
        ...
        g['score']=g['median_return_pct']-0.9*abs(g['median_mdd_pct'])-0.4*abs(g['worst_mdd_pct'])+4*g['windows_10x_plus']+12*g['windows_30x_plus']; grids.append(g)
    ...
    summary={'method':'Turbo v3 fast: acceleration rank, top2/leader switch, winner-only anti-martingale, trailing lock, 2-stage DD brake; 10 crypto, rolling 2y/180d, fee 0.05%', 'candidate_count':len(gd),'best_overall':gd.iloc[0].to_dict(),'best_mdd_constrained':safe.iloc[0].to_dict() if len(safe) else None,'highest_window':top.iloc[0].to_dict()}
