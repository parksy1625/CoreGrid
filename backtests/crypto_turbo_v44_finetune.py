...
OUT=Path('turbo_v44_output'); OUT.mkdir(exist_ok=True)
...
def run(idx,A,cfg,i0,i1):
    ...
    for i in range(i0+1,i1+1):
        ...
        if len(inds):
            ...
            conc=len(top)==1 or (len(top)>=2 and score[top[0]]-score[top[1]]>=0.035 and accel[top[0]]>0)
            picks=[top[0]] if conc else list(top); base_total=1.0 if conc else 1.2
            scales=np.clip(cfg['vol_target']/np.array([vol[j] for j in picks]),0.45,cfg['vol_cap'])
            raw=np.ones(len(picks))*(base_total/len(picks))*scales
            for q,j in enumerate(picks):
                if np.isfinite(entry[j]) and entry[j]>0:
                    gain=c0[j]/entry[j]-1
                    if gain>=0.10: raw[q]=max(raw[q],1.5*scales[q])
                    if gain>=0.25: raw[q]=max(raw[q],2.25*scales[q])
                desired[j]=raw[q]
        dd=eq/peak-1
        ...
        for j in np.where(np.isfinite(entry))[0]:
            if np.isfinite(c0[j]):
                ...
                if tpeak[j]/entry[j]-1>=0.35 and c0[j]/tpeak[j]-1<=-0.16: desired[j]=0.0
        tot=desired.sum()
        if tot>2.5: desired*=2.5/tot
...
def main():
    ...
    cfgs=[]
    for vt in [0.95,1.00,1.05,1.10,1.15,1.20]:
      for cap in [1.35,1.45,1.55,1.65]:
       for d1,d2,b1,b2 in [(0.10,0.18,0.65,0.30),(0.11,0.20,0.70,0.35),(0.12,0.22,0.70,0.35)]:
        cfgs.append({'vol_target':vt,'vol_cap':cap,'dd1':d1,'dd2':d2,'brake1':b1,'brake2':b2})
    rows=[]; grids=[]
    for ci,cfg in enumerate(cfgs,1):
        ...
        d=pd.DataFrame(vals)
        g={**cfg,'windows':len(d),'median_return_pct':d.return_pct.median(),'best_return_pct':d.return_pct.max(),'worst_return_pct':d.return_pct.min(),'median_mdd_pct':d.mdd_pct.median(),'worst_mdd_pct':d.mdd_pct.min(),'profitable_rate_pct':(d.return_pct>0).mean()*100,'windows_10x_plus':int((d.multiple>=10).sum()),'windows_30x_plus':int((d.multiple>=30).sum()),'windows_100x_plus':int((d.multiple>=100).sum()),'windows_200x_plus':int((d.multiple>=200).sum()),'windows_300x_plus':int((d.multiple>=300).sum()),'windows_500x_plus':int((d.multiple>=500).sum()),'liquidations':int(d.liquidated.sum())}
        g['score']=g['median_return_pct']-0.75*abs(g['median_mdd_pct'])-0.20*abs(g['worst_mdd_pct'])+5*g['windows_10x_plus']+15*g['windows_30x_plus']+45*g['windows_100x_plus']+80*g['windows_200x_plus']; grids.append(g)
    gd=pd.DataFrame(grids).sort_values(['windows_500x_plus','windows_300x_plus','windows_200x_plus','windows_100x_plus','score'],ascending=False)
    rd=pd.DataFrame(rows); gd.to_csv(OUT/'grid.csv',index=False); rd.to_csv(OUT/'rolling_2y.csv',index=False); rd.sort_values('multiple',ascending=False).head(40).to_csv(OUT/'top_windows.csv',index=False)
    safe=gd[(gd.median_mdd_pct>=-50)&(gd.worst_mdd_pct>=-65)]
    summary={'method':'Turbo v4.4 fine search around v4.2; 72 configs, 10 crypto, rolling 2y/180d, fee 0.05%','candidate_count':len(gd),'best_overall':gd.iloc[0].to_dict(),'best_safe':safe.iloc[0].to_dict() if len(safe) else None,'highest_window':rd.sort_values('multiple',ascending=False).iloc[0].to_dict()}
