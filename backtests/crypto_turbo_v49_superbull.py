...
OUT=Path('turbo_v49_output'); OUT.mkdir(exist_ok=True)
...
def prep(df):
    ...
    return pd.DataFrame({'open':df.open,'close':c,'score':score,'accel':accel,'bull':bull.astype(float),'vol':vol,'m7':m7,'m30':m30})
...
def build_arrays():
    ...
    keys=['open','close','score','accel','bull','vol','m7','m30']
...
def run(idx,A,cfg,i0,i1):
    ...
    strong_hold=0; super_hold=0; super_hours=0
    for i in range(i0+1,i1+1):
        p=i-1; c0=A['close'][p]; c1=A['close'][i]; score=A['score'][p]; accel=A['accel'][p]; bull=A['bull'][p]; vol=A['vol'][p]; m7=A['m7'][p]; m30=A['m30'][p]
        ...
        strong=False; super_bull=False
        if len(inds):
            ords=inds[np.argsort(score[inds])[::-1]]; top=ords[:2]; leader=top[0]
            if breadth>=cfg['breadth_thr'] and accel[leader]>=cfg['accel_thr'] and score[leader]>=-0.01: strong_hold=24
            elif strong_hold>0: strong_hold-=1
            strong=strong_hold>0
            super_trigger=(breadth>=cfg['super_breadth'] and accel[leader]>=cfg['super_accel'] and m7[leader]>=cfg['super_m7'] and m30[leader]>=cfg['super_m30'])
            if super_trigger: super_hold=cfg['super_hold']
            elif super_hold>0: super_hold-=1
            super_bull=super_hold>0
            if super_bull: super_hours+=1
            conc=len(top)==1 or (len(top)>=2 and score[top[0]]-score[top[1]]>=0.03 and accel[top[0]]>0)
            picks=[top[0]] if conc else list(top)
            base_total=(1.20 if strong else 1.0) if conc else (1.40 if strong else 1.2)
            ...
            for q,j in enumerate(picks):
                if np.isfinite(entry[j]) and entry[j]>0:
                    ...
                    if super_bull and gain>=cfg['super_gain1']: raw[q]=max(raw[q],cfg['super_p1']*scales[q])
                    if super_bull and gain>=cfg['super_gain2']: raw[q]=max(raw[q],cfg['super_p2']*scales[q])
                desired[j]=raw[q]
        dd=eq/peak-1
        ...
        max_total=cfg['super_max'] if super_bull else (4.6 if strong else 3.1); tot=desired.sum()
        ...
        eq*=1.+np.dot(w,rets)
        if not np.isfinite(eq) or eq<=0: return {'return_pct':-100.,'multiple':0.,'mdd_pct':-100.,'liquidated':1,'super_hours':super_hours}
    ...
    return {'return_pct':(eq-1)*100,'multiple':eq,'mdd_pct':maxdd*100,'liquidated':0,'super_hours':super_hours}
...
def main():
    ...
    cfgs=[]
    for sb in [0.50,0.60,0.70]:
      for sa in [0.02,0.05,0.08]:
       for sm7 in [0.08,0.12]:
        for sp1 in [5.5,6.5]:
         for sp2 in [7.0,8.0]:
          for smax in [5.0,5.8]:
           cfgs.append({'breadth_thr':0.30,'accel_thr':-0.01,'super_breadth':sb,'super_accel':sa,'super_m7':sm7,'super_m30':0.20,'super_hold':48,'super_gain1':0.70,'super_gain2':1.20,'super_p1':sp1,'super_p2':sp2,'super_max':smax})
    rows=[]; grids=[]
    ...
    gd.to_csv(OUT/'grid.csv',index=False); rd.to_csv(OUT/'rolling_2y.csv',index=False); rd.sort_values('multiple',ascending=False).head(120).to_csv(OUT/'top_windows.csv',index=False)
    ...
    summary={'method':'Turbo v4.9 V4.8 median core + super-bull tail mode; target best 2y >=3000x and median >=200%; rolling 2y/180d, fee 0.05%','candidate_count':len(gd),'joint_target_count':len(joint),'target_3000_count':len(t3),'median200_count':len(med),'best_overall':gd.iloc[0].to_dict(),'best_joint':joint.iloc[0].to_dict() if len(joint) else None,'best_3000':t3.iloc[0].to_dict() if len(t3) else None,'best_median200':med.sort_values('median_return_pct',ascending=False).iloc[0].to_dict() if len(med) else None,'highest_window':rd.sort_values('multiple',ascending=False).iloc[0].to_dict()}
