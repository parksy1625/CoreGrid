...
OUT=Path('turbo_v53_output'); OUT.mkdir(exist_ok=True)
...
def main():
    ...
    cfgs=[]
    for alloc in [.15,.20,.25,.30]:
      for b in [.75,.85]:
       for m30 in [.60,.90]:
        for lev in [2.5,3.0,3.5]:
         for hold in [8,16]:
          for near in [.985,.995]:
           cfgs.append({'alloc':alloc,'breadth':b,'m7':.15,'m30':m30,'near':near,'accel':.015,'lev':lev,'lev2':lev*1.35,'gain2':.50,'cap':lev*1.35,'hold':hold})
    rows=[]; grids=[]
    for ci,cfg in enumerate(cfgs):
        ...
        g['median250_target']=int(g['median_return_pct']>=250 and g['liquidations']==0)
        g['joint_target']=int(g['best_return_pct']>=299900 and g['median_return_pct']>=250 and g['liquidations']==0)
        g['score']=6500*g['joint_target']+1800*g['median250_target']+.0018*g['best_return_pct']+2.5*g['median_return_pct']+350*g['windows_3000x_plus']+110*g['windows_1000x_plus']-.45*abs(g['median_mdd_pct'])-.12*abs(g['worst_mdd_pct'])
        grids.append(g)
        if ci%16==0: print('progress',ci+1,'/',len(cfgs),flush=True)
    gd=pd.DataFrame(grids).sort_values(['joint_target','median250_target','median_return_pct','windows_3000x_plus','score'],ascending=False)
    ...
    joint=gd[(gd.best_return_pct>=299900)&(gd.median_return_pct>=250)&(gd.liquidations==0)]
    t3=gd[(gd.best_return_pct>=299900)&(gd.liquidations==0)]
    med=gd[(gd.median_return_pct>=250)&(gd.liquidations==0)]
    summary={'method':'Turbo v5.3 V5.2 isolated dual-sleeve fine-tune; target median>=250%, best>=3000x, zero total liquidations','candidate_count':len(gd),'joint_target_count':len(joint),'target_3000_count':len(t3),'median250_count':len(med),'best_overall':gd.iloc[0].to_dict(),'best_joint':joint.iloc[0].to_dict() if len(joint) else None,'best_3000':t3.sort_values('best_return_pct',ascending=False).iloc[0].to_dict() if len(t3) else None,'best_median250':med.sort_values('median_return_pct',ascending=False).iloc[0].to_dict() if len(med) else None,'highest_window':rd.sort_values('multiple',ascending=False).iloc[0].to_dict()}
